...
// /*   Updated: 2023/06/05 16:39:45 by gd-harco         ###   ########lyon.fr   */
// /*                                                                            */
// /* ************************************************************************** */
...
// // 	tmp = f_token;
// // 	cmd.io_fd[STDIN_FILENO] = dup(STDIN_FILENO);
// // 	cmd.io_fd[STDOUT_FILENO] = dup(STDOUT_FILENO);
// // 	while (tmp && tmp->type != PIPE)
// // 	{
// // 		if (tmp->type == FILE_IN || tmp->type == HERE_DOC)
// // 			set_input_fd(*tmp, cmd.io_fd, exec_data);
// // 		else if (tmp->type == FILE_OUT || tmp->type == FILE_OUT_APPEND)
// // 			set_output_fd(*tmp, cmd.io_fd);
